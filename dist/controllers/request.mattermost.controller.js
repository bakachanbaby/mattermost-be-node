...
const handleAddRequest = async (req, res) => {
  try {
    ...
    try {
      ...
      await axios.post(urlAddRequest, reqRequest).then(async e => {
  ...
  try {
    ...
    if (lstBot[0].channelIds[0] === channel_id) {
      ...
      const fields = [{
        ...
        title: "Lĩnh vực",
        value: request.category.description,
        short: true
      }, {
        title: "Trạng thái",
      ...
      if (request.comments && request.comments.content) {
        fields.push({
          title: "Bình luận",
          value: request.comments.content,
          short: false
        });
      }
      const messageData = {
    ...
    if (lstBot[0].channelIds[0] === channel_id) {
      ...
      if (!flat) {
        ...
        try {
          ...
          const directMessage = await axios.post(DIRECT_URL, [botId, admin.userId], {
            headers: {
              'Authorization': `Bearer ${MATTERMOST_ACCESS_BOT_TP}`,
              'Content-Type': 'application/json'
            }
...
const handleRequestToEdit = async (req, res) => {
  try {
    ...
    let access = channel_id === lstBot[0].channelIds[0] ? MATTERMOST_ACCESS : MATTERMOST_ACCESS_BOT_TP;
    let lstRequest = await RequestModal.find();
    if (access === MATTERMOST_ACCESS) lstRequest = lstRequest.filter(request => request.status === REQUEST_STATUS.IDLE);else lstRequest = lstRequest.filter(request => request.status === REQUEST_STATUS.SENT);
...
const handleEditRequest = async (req, res) => {
  try {
    ...
    try {
      ...
      await axios.put(`${urlAddRequest}/${req.params.id}`, reqRequest).then(async e => {
        console.log(e.data);
  ...
  try {
    ...
    if (lstBot[0].channelIds[0] === channel_id) {
      if (request && request.status !== REQUEST_STATUS.IDLE) {
        const messageData = {
          channel_id: channel_id,
          message: `Kiến nghị mã **${request.code}** không có trong danh sách, không thể xóa`
        };
        await axios.post(MESSAGE_URL, messageData, {
          headers: {
            'Authorization': `Bearer ${MATTERMOST_ACCESS}`,
...
const handleConfirmDeleteRequest = async (req, res) => {
  try {
    ...
    let access = channel_id === lstBot[0].channelIds[0] ? MATTERMOST_ACCESS : MATTERMOST_ACCESS_BOT_TP;
    try {
      await axios.delete(`${urlAddRequest}/${req.params.id}`).then(async e => {
        console.log(e.data);
        await axios.delete(`${POST_URL}/${post_id}`, {
          headers: {
            'Authorization': `Bearer ${access}`
  ...
  try {
    ...
    try {
      const messageData = {
        ...
        props: {
          attachments: [{
            text: "### Chọn kiến nghị cần xin kiến nghị",
            actions: [{
              "id": "actionoptions",
              "name": "Vui lòng chọn kiến nghị",
              "integration": {
  ...
  try {
    ...
    try {
      const userSend = await UserModel.findOne({
        userId: req.body.submission.user
      });
      ...
      const botId = lstBot[1].botUserId;
      const directMessage = await axios.post(DIRECT_URL, [botId, userSend.userId], {
        headers: {
          'Authorization': `Bearer ${MATTERMOST_ACCESS_BOT_TP}`,
          'Content-Type': 'application/json'
        }
...
const handleCommentRequest = async (req, res) => {
  try {
    ...
    try {
      ...
      await axios.put(`${urlAddRequest}/${req.params.id}/comment`, reqRequest).then(async e => {
        console.log(e.data);
        ...
        // Thay đổi trạng thánh kiến nghị là SENT
        const request = await RequestModal.findOne({
          _id: e.data._id
        });
        request.status = REQUEST_STATUS.SENT;
        ...
        try {
          ...
          const messageData = {
            channel_id: channel_id,
            message: `Bạn đã bình luận kiến nghị với mã **${e.data.code}**`
          };
          await axios.post(MESSAGE_URL, messageData, {
            headers: {
              'Authorization': `Bearer ${MATTERMOST_ACCESS_BOT_TP}`,
            ...
            props: {
              attachments: [{
                ...
                fields: [{
                  ...
                  title: "Ngày nhận",
                  value: e.data.receivedDate,
                  short: true
                }, {
                  title: "Lĩnh vực",
                  value: e.data.category.description,
                  short: true
                }, {
                  title: "Trạng thái",
                  value: getStatus(e.data.status),
                  short: true
                }, {
                  title: "Nội dung",
                ...
                actions: [{
                  ...
                  type: 'button',
                  integration: {
                    url: `${NGROK_URL}/api/request-mattermost/open-edit-request/${e.data.code}`,
                    context: {
                      action: 'edit'
                    }
                  },
